...
/**
 * @author john huang 2015年2月28日 下午10:20:37 
 * 本类主要做为与c写的des进行匹配使用
 */
public class Des2cUtil {
	 ...
	 * @return
	 */
	private static String DES_1(String source, String key, int type) {
	 ...
	 */
	public static String encryptDes(String source, String key) throws ToolsException
	{
		if (key.length() != 16)
			return null;
		String addzeros = "";
		if(source.length() % 16 !=0){
			//明文内容不为16时，后补0至16的倍数
			addzeros = "0000000000000000".substring(source.length() % 16,16);
			source = source+addzeros;
		}
		
		 try {
	           byte[] theKey = null;
	           byte[] theMsg = null;
	           theMsg = hexToBytes(source);
	           ...
	           throw new ToolsException("DES加密失败",e);
	        }
	 ...
	 * @throws ToolsException 
	 */
	public static String decryptDes(String source,String key) throws ToolsException{
		if (source.length() != 16 )
			throw new ToolsException("密文非16位");
		if ( key.length() != 16)
			throw new ToolsException("密钥非16位");
		try {
	           byte[] theKey = null;
	           byte[] theMsg = null;
	           theMsg = hexToBytes(source);
	           ...
	           return bytesToHex(theCph);
	        	...
	        	throw new ToolsException("DES解密失败",e);
	        }
	}
	/**
	 * des加密
...
//	/**
//	 * 三重DES算法(双�?长密�?32byte)) 密钥K1和K2 1、先用K1加密明文 2、接�?��K2对上�?��的结果进行解�?
//	 * 3、然后用K1对上�?��的结果进行加�?
//	 * 
//	 * @param source
//	 * @param key
...
//	}
	...
	public static void main(String[] args) throws ToolsException {
		String encryptDes = encryptDes("1234560000000000", "1111111111111111");
		encryptDes = encryptDes("06123456FFFFFFFF", "1111111111111111");
		System.out.println(String.format("明文=1234560000000000,密钥=1111111111111111,加密后=%s", encryptDes));
		String decryptDes = decryptDes("E2CB0E2CB6C1ACBD", "1111111111111111");
		System.out.println(String.format("密文=E2CB0E2CB6C1ACBD,密钥=1111111111111111,解密后=%s", decryptDes));
	}
